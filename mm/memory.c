 ...
 */
ulong get_free_page(int real_space)
{
/*
...
register ulong __res asm("ax");
ulong compare_addr = KERNEL_LINEAR_ADDR_SPACE;
ulong paging_num = PAGING_PAGES;  /* Granularity: 4K */
ulong paging_end = mem_map+(PAGING_PAGES-1);
ulong paging_start = LOW_MEM;     /* Granularity: Byte */
...
void vm_exit(ulong  exit_reason_no, cpuid_exit_info* exit_info) {
	exit_reason_cpuid* exit_reason_cpuid_info_p = (exit_reason_cpuid*) VM_EXIT_REASON_CPUID_INFO_ADDR;
	exit_reason_cpuid_info_p->exit_reason_no = exit_reason_no;
	exit_reason_cpuid_info_p->exit_info = *exit_info;
	__asm__ ("cpuid\n\r"::);
}

void free_ept_page(ulong guest_page_phy_addr) {
	vm_exit(VM_EXIT_REASON_CPUID_FOR_FREE_EPT_PAGE,(cpuid_exit_info*)(&guest_page_phy_addr));
}
...
int copy_page_tables(unsigned long from,unsigned long to,long size,struct task_struct* new_task)
{
	...
	unsigned long *new_dir_page = (unsigned long*)get_free_page(PAGE_IN_REAL_MEM_MAP_FOR_GUEST_CR3);  /* 为新进程分配一页物理内存用于存储目录表 */
	...
	new_task->tss.cr3 = (ulong)new_dir_page;
	/*
